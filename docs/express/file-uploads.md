...
Sharp is a high-performance image processing library for Node.js. Use it for resizing, optimization, format conversion, and more.
...
app.post('/upload-resize', upload.single('image'), async (req, res) => {
  try {
    ...
    const outputPath = `uploads/resized/${Date.now()}-${req.file.originalname}`;
...
// Example usage:
// GET /images/photo.jpg?width=200&height=150
```
...
app.get('/:name', async (req, res) => {
  try {
    ...
    // Extract image number (e.g., "img1" -> "1")
...
// Resize with different fit options
...
await sharp('input.jpg')
  .resize(300) // Width only, height auto
  .toFile('output.jpg');
...
console.log(metadata); // { width, height, format, etc. }
```
...
app.post('/upload-multi-size', upload.single('image'), async (req, res) => {
  try {
    const sizes = {
    ...
    for (const [sizeName, dimensions] of Object.entries(sizes)) {
      const outputPath = path.join('uploads', sizeName, filename);
    ...
    res.json({
      message: 'Image uploaded in multiple sizes',
      urls: urls
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.post('/upload-optimized', upload.single('image'), async (req, res) => {
  try {
    const outputPath = `uploads/optimized/${Date.now()}-${req.file.originalname}`;
...
app.get('/img/:filename', async (req, res) => {
  try {
    ...
    // Check cache if resizing is requested
    if (width && height) {
...
// Example requests:
// GET /img/photo.jpg                                -> Original
// GET /img/photo.jpg?width=300&height=200           -> Resized JPEG
// GET /img/photo.jpg?width=300&height=200&format=webp -> Resized WebP
```
